<template>
  <div class="mod-usercoupon">
    <el-form :inline="true" :model="searchForm" @keyup.enter.native="getDataList(1)">
      ...
      <el-form-item>
        <el-select v-model="searchForm.status" clearable placeholder="使用状态">
          <el-option
            key="0"
            label="未使用"
            value="0">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="使用的订单编码">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="showOrderDetails(scope.row.orderId)">{{scope.row.orderSn}}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column
