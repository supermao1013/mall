		...
		<view class="tui-keyboard__grids">
			<view class="tui-keyboard__left">
				<view class="tui-keyboard__grid" :class="{'tui-flex__2':item=='0'}" v-for="(item,index) in keyArr" :key="index">
					<view class="tui-key__item" :style="{color:darkMode?'#D1D1D1':color,backgroundColor:darkMode?'#2C2C2C':keyBackground}"
					 :hover-class="!isDecimal && index==10?'':'tui-key__item-active'" :hover-stay-time="150" @tap.stop="handleClick"
			...
			<view class="tui-keyboard__right">
				...
				<view class="tui-keyboard__grid tui-dk__btn">
					<view class="tui-key__item" :class="{'tui-dkBtn__disabled':disabled}" @tap.stop="confirm" :style="{color:buttonColor,backgroundColor:buttonBackground,fontSize:buttonSize,fontWeight:buttonFontBold?'bold':'normal'}"
