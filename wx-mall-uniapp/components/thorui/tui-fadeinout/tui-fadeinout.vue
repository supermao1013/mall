<template>
	<view>
		<view ref="ani" :animation="animationData" class="message round bg-gradual-orange flex justify-start shadow" style="padding: 3px;"
		 :style="{ top: top + 'px', left: left + 'px' }" v-if="show">
			<view v-if="content[img]" class="cu-avatar cu-a-sm round" :style="{ backgroundImage: `url(${content[img]})` }">
				<!-- #ifdef APP-NVUE -->
				<image :src="content[img]" class="avatarimg"></image>
				<!-- #endif -->
			</view>
			<view class="padding-lr-sm flex align-center">
				<text class="text-sm">{{ content[title] + extra}}</text>
			</view>
		</view>
	</view>
</template>
