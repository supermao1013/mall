<template>
	<view class="container">
        ...
        <view v-for="(cart,index) in checkedGoodsList" :key="index">
          <tui-list-cell :hover="false" :arrow="true" @click="switchShops(cart.shopsSn)">
            <view class="tui-goods-title">
              <tui-icon name="shop" :size="22"></tui-icon><text class="shops-name">{{cart.name||''}}</text>
            </view>
          </tui-list-cell>
          <block v-for="(item, i) in cart.cartList" :key="i">
            <tui-list-cell :hover="false" padding="0">
              <view class="tui-goods-item">
                <image :src="item.listPicUrl" class="tui-goods-img" :data-id="item.goodsId" @tap="goodsDetail"></image>
          ...
          <tui-list-cell :hover="false" :lineLeft="false" padding="0">
            <view class="tui-remark-box tui-padding tui-flex">
              <view>订单备注</view>
              <input type="text" class="tui-remark" v-model="cart.postscript" placeholder="选填: 请先和商家协商一致" auto-focus
				...
				<tui-list-cell :hover="false" @click="navigateToCoupon">
					<view class="tui-padding tui-flex">
						<view>优惠券</view>
						<view class="coupon">
							<text v-if="!couponId">选择优惠券</text>
							<text v-else>- ￥{{sumSubPrice}}</text>
						</view>
					</view>
				</tui-list-cell>
				<tui-list-cell :hover="false">
			...
			<view class="tui-top">
				<tui-list-cell :hover="false" v-for="(item, index) in cartArr" :key="index" @click="radioChange({'index':item.value})">
					<view class="tui-padding tui-flex">
            <view>
		...
		<view class="tui-tabbar">
			...
			<view class="tui-pr25">
				<tui-button width="200rpx" height="70rpx" type="danger" shape="circle" @click="submitOrder">确认支付</tui-button>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			updateCouponId(newValue) {
				this.couponId = newValue;
			},
        ...
        uni.navigateTo({
          ...
          checkedGoodsList: that.checkedGoodsList,
			...
			navigateToCoupon: function() {
				let that = this
				uni.navigateTo({
					url: '/pages/shopping/selectCoupon/selectCoupon?checkedGoodsIdList=' + that.checkedGoodsIdList +
						'&checkedBrandIdList=' + that.checkedBrandIdList,
				})
			}
		},
		onShow: function() {
			...
			that.couponId = uni.getStorageSync('couponId')
      ...
      that.payType = 'zfb'
      // #endif
      that.buyType = that.isBuy ? 'buy' : 'cart'
