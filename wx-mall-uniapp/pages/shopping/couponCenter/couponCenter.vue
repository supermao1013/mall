<template>
	<view class="container">
		<tui-show-empty v-if="couponList.length<=0" showType="NO_SUIT_COUPON"></tui-show-empty>
		<view v-else>
			<image src="/static/images/mall/banner_coupon.png" mode="widthFix" class="tui-coupon-banner"></image>
			<view class="tui-coupon-item tui-top20" v-for="(item, index) in couponList" :key="index">
				...
				<image src="/static/images/mall/img_coupon_over_3x.png" class="tui-coupon-sign" mode="widthFix"
					v-if="item.sendCount>=item.totalCount"></image>
				<view class="tui-coupon-item-left">
					<view :class="'tui-coupon-price-box ' + (item.sendCount>=item.totalCount?'tui-color-grey':'')">
						...
						<view class="tui-coupon-price tui-price-small" v-if="item.couponType==1">{{item.subPrice}}
						</view>
						<view class="tui-coupon-price tui-price-small" v-if="item.couponType==2">{{item.discount}}
						</view>
				...
				<view class="tui-btn-box" v-if="item.sendCount<item.totalCount">
					<tui-button type="danger" @click="getCouponUser(item.id)" width="146rpx" height="52rpx" :size="24"
						shape="circle" :plain="false">立即领取</tui-button>
				</view>
			</view>
		</view>
	</view>
