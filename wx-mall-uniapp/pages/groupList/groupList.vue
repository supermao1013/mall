				...
				<view class="tui-searchbox tui-search-mr" :style="{ marginTop: inputTop + 'px' }">
					...
					<view class="tui-search-key">
						<input confirm-type="search" v-model.trim="keyword" :auto-focus="searchStatus==1"
							...
							@tap="searchStatus=1" />
						<tui-icon name="shut" :size="15" color="#999" v-if="keyword" @click="clearKeyword"></tui-icon>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			uni.setStorageSync("navUrl", "/pages/groupList/groupList");
			let obj = {};
			// #ifdef MP-WEIXIN
		...
		methods: {
			...
			categorySelectSerach() {
				let that = this;
				that.attrIndex = 2
				that.dropBrandScreenShow = false;
